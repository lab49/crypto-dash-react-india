...
const CurrencyDropdown = ({ value: selectedValue, onChangeHandler, optionList, keyPrefix }) => {
    ...
    return (
        <div className="dropdown w-100" onClick={toggleOpen}>
            <button className="d-flex justify-content-between align-items-center w-100 btn btn-lg btn-secondary dropdown-toggle"
                    type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                {
                    optionsMap ?
                        <CurrencyRow name={optionsMap.get(selectedValue).label} symbol={optionsMap.get(selectedValue).symbol}/>
                    : selectedValue
                }
            </button>
